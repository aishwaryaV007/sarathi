      ...
      <Card className="w-full max-w-[380px] border-sarathi-line shadow-[0_1px_2px_rgba(16,42,79,.08)] rounded-[14px] overflow-hidden">
        ...
        <CardContent className="pb-8 px-8">
          <button 
            ...
            className={`inline-flex items-center justify-center bg-white border-[1.5px] border-sarathi-line hover:bg-gray-50 text-sarathi-ink font-semibold text-[15.5px] h-[48px] w-full rounded-[8px] transition-colors shadow-sm ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {isLoading ? (
              <div className="w-5 h-5 mr-3 border-2 border-sarathi-blue border-t-transparent rounded-full animate-spin" />
            ) : (
  ...
  return (
    <Suspense fallback={
      <div className="min-h-[calc(100vh-140px)] flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-sarathi-blue border-t-transparent rounded-full animate-spin" />
      </div>
    }>
      <LoginForm />
    </Suspense>
